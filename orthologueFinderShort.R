...
for(i in c(9:13)){
  ...
  other_further_filtered_old_3_5 <- other_further_filtered_3_5 %>%
    filter((strand == 5 & qend > 616 & lag(qseqid) == qseqid) |
             (strand == 3 & qend > lead(616) & lead(qseqid) == qseqid) |
  ...
  write_tsv(x = present_bound_resolved, path = paste0("extended_fastas/resolved_locations/small_", species_name, "_in_", s_species, ".tsv"), col_names = F)
...
# write to file
write_tsv(x = collated_info, path = paste0("extended_fastas/collated_results/", species_name, "_small_collated_results.tsv"), col_names = T)
...
table(collated_info$Anser_indicus)
table(collated_info$Anser_cygnoides_domesticus)
table(collated_info$Anser_brachyrhynchus)
table(collated_info$Anas_zonorhyncha)
table(collated_info$Anas_platyrhynchos)

# determine whether ancestral or recent (needs to be adjusted for each species)
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Anser_brachyrhynchus == 0 & Anas_zonorhyncha == 0) ~ 0,
    (Anser_brachyrhynchus == 1 & Anas_zonorhyncha == 0) ~ 1,
    (Anser_brachyrhynchus == 1 & Anas_zonorhyncha == 1) ~ 2,
    TRUE ~ -1
  )
  ) %>%
  arrange(round_jc)
...
divergence_plot <- ggplot(data = counts, aes(round_jc, Freq, fill = state)) + geom_bar(stat = "identity") +
  scale_x_continuous(name = "Jukes-Cantor distance", expand = c(0,0), limits = c(-0.002, 0.45)) +
  theme_bw() +
  scale_fill_manual(labels = c("Ancestral",  "Since Anser", "Since Anas"),
                    values = brewer.pal(n = 4, name = 'BrBG')) +
  scale_y_continuous(limits = c(0, max(counts_max$sumFreq) * 1.05), expand = c(0,0), name = "Insertions") +
