...
# set query species
species_name <- species_list$species_name[95]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Strigops_habroptilis == 0 & Melopsittacus_undulatus == 0 & Amazona_collaria == 0) ~ 1,
    (Strigops_habroptilis == 1 & Melopsittacus_undulatus == 0 & Amazona_collaria == 0) ~ 2,
    (Strigops_habroptilis == 1 & Melopsittacus_undulatus == 0 & Amazona_collaria == 1) ~ 3,
    (Strigops_habroptilis == 1 & Melopsittacus_undulatus == 1 & Amazona_collaria == 1) ~ 4,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Strigops", "Since Melopsittacus", "Since Amazona"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2,5:6)]) +
  scale_y_continuous(limits = c(0, 2000), expand = c(0,0), name = "Insertions") +
...
species_name <- species_list$species_name[95]
genome_name <- species_list$genome_name[95]
