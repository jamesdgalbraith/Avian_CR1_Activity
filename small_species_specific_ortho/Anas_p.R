...
# set query species
species_name <- species_list$species_name[9]
...
for(i in c(9:13, 58)){
  ...
  write_tsv(present_bound_resolved, paste0("extended_fastas/resolved_locations/small_", species_name, "_in_", s_species, ".tsv"), col_names = F)
...
# write to file
write_tsv(x = collated_info, paste0("extended_fastas/collated_results/", species_name, "_small_collated_results.tsv"), col_names = T)
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    ((Anser_brachyrhynchus == 0 | Anser_indicus == 0 | Anser_cygnoides_domesticus == 0) & Anas_zonorhyncha == 0 & Gallus_gallus == 0) ~ 1,
    ((Anser_brachyrhynchus == 0 | Anser_indicus == 0 | Anser_cygnoides_domesticus == 0) & Anas_zonorhyncha == 0 & Gallus_gallus == 1) ~ 2,
    ((Anser_brachyrhynchus == 1 | Anser_indicus == 1 | Anser_cygnoides_domesticus == 1) & Anas_zonorhyncha == 0 & Gallus_gallus == 1) ~ 3,
    ((Anser_brachyrhynchus == 1 | Anser_indicus == 1 | Anser_cygnoides_domesticus == 1) & Anas_zonorhyncha == 1) ~ 4,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Anser", "Since Anas"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2, 6)]) +
...
species_name <- species_list$species_name[9]
genome_name <- species_list$genome_name[9]
