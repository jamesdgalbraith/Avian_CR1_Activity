...
# set query species
species_name <- species_list$species_name[69]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) & Acanthisitta_chloris == 0 & Melopsittacus_undulatus == 0 ~ 0, # since parrots
    (Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) & Acanthisitta_chloris == 0 & Melopsittacus_undulatus == 1 ~ 1, # since parrots
    (Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) & Acanthisitta_chloris == 1 & Melopsittacus_undulatus == 1 ~ 2, # since NZ wrens
    Corvus_brachyrhynchos == 1 & Taeniopygia_guttata == 1 & Acanthisitta_chloris == 0 ~ 3, # since parrots, deletion in oscines
    Corvus_brachyrhynchos == 1 & Taeniopygia_guttata == 1 & Acanthisitta_chloris == 1 ~ 4, # since oscines
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Since Acanthisitta", "Since Taeniopygia"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(3,4)]) +
  scale_y_continuous(limits = c(0, 110), expand = c(0,0), name = "Insertions") +
...
# species_name <- species_list$species_name[69]
# genome_name <- species_list$genome_name[69]
# 
