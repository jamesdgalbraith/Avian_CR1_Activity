...
# set query species
species_name <- species_list$species_name[44]
...
for(i in c(1, 44, 45, 54, 69, 71, 110, 116)){
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Corvus_cornix_cornix == 0 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0) & Manacus_vitellinus == 0 & Acanthisitta_chloris == 0 & Melopsittacus_undulatus == 1 ~ 1,
    Corvus_cornix_cornix == 0 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0) & Manacus_vitellinus == 0 & Acanthisitta_chloris == 1 ~ 2,
    Corvus_cornix_cornix == 0 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0) & Manacus_vitellinus == 1 & Acanthisitta_chloris == 1 ~ 3,
    Corvus_cornix_cornix == 0 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0) & Manacus_vitellinus == 1 & Acanthisitta_chloris == 1 ~ 4,
    Corvus_cornix_cornix == 0 & (Taeniopygia_guttata == 1 | Zonotrichia_albicollis == 1)  & Manacus_vitellinus == 1 ~ 5,
    Corvus_cornix_cornix == 1 & (Taeniopygia_guttata == 1 | Zonotrichia_albicollis == 1)  & Manacus_vitellinus == 1 ~ 6,
    TRUE ~ -1 # unclear
  )
  ) %>%
  inner_join(reciprocal_blast_out) %>%
  # filter(!state %in% c(-1, 0)) %>%
  base::unique()
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Since Acanthisitta", "Since Manacus", "Since Taeniopygia",  "Since Corvus"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1,2,4)]) +
  scale_y_continuous(limits = c(0, 70), expand = c(0,0), name = "Insertions") +
...
collated_info_3 <- collated_info_2 %>% filter(state == 7)

table(collated_info_3$state, collated_info_3$subfamily)
...
species_name <- species_list$species_name[44]
genome_name <- species_list$genome_name[44]
...
collated_info_3_ranges <- collated_info_3 %>%
  dplyr::select(qseqid, qseqnames, ostart, oend, ostrand, subfamily, sseqid, state) %>%
  mutate(ostart = ostart - 500, oend = oend + 500) %>%
  dplyr::rename(seqnames = qseqnames, strand = ostrand, start = ostart, end = oend) %>%
  as_granges()

collated_info_3_seq <- getSeq(genome_seq, collated_info_3_ranges)
names(collated_info_3_seq) <- paste0(species_name, "#", collated_info_3_ranges$qseqid)

for(i in c(45, 69, 110, 116)){
  
  s_species_name <- species_list$species_name[i]
  s_genome_name <- species_list$genome_name[i]
  print(s_species_name)
  genome_seq <- readDNAStringSet(paste0("~/Genomes/Birds/", s_species_name, "/", s_genome_name))
  names(genome_seq) <- sub(" .*", "", names(genome_seq))
  gc()
  present_bound_resolved_ranges <- read_tsv(file = paste0("extended_fastas/resolved_locations/small_", species_name, "_in_", s_species_name, ".tsv"),
                                            col_names = c("seqnames", "start", "end", "qseqid", "strand", "state")) %>%
    filter(qseqid %in% collated_info_3_ranges$qseqid) %>%
    as_granges()
  present_bound_resolved_seq <- getSeq(genome_seq, present_bound_resolved_ranges)
  names(present_bound_resolved_seq) <- paste0(s_species_name, "#", present_bound_resolved_ranges$qseqid)
  collated_info_3_seq <- c(collated_info_3_seq, present_bound_resolved_seq)
  
}

for(i in 1:length(collated_info_3_ranges$qseqid)){
  to_align <- collated_info_3_seq[grepl(sub("\\(.\\)", "", collated_info_3_ranges$qseqid[i]), names(collated_info_3_seq))]
  writeXStringSet(to_align, "temp/temp.fa")
  system(paste0("mafft --adjustdirection --localpair temp/temp.fa > extended_fastas/alignments/", species_name, "_", i, "_", collated_info_3_ranges$state[i], ".fasta"))
}
