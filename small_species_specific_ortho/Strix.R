...
# set query species
species_name <- species_list$species_name[107]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Bubo_bubo == 0 & Otus_sunia == 0 & Athene_cunicularia == 0 & Tyto_alba == 0 ~ 0, #
    Bubo_bubo == 0 & Otus_sunia == 0 & Athene_cunicularia == 0 & Tyto_alba == 1 ~ 1, #
    Bubo_bubo == 0 & Otus_sunia == 0 & Athene_cunicularia == 1 & Tyto_alba == 1 ~ 2, #
    Bubo_bubo == 0 & Otus_sunia == 1 & Athene_cunicularia == 1 ~ 3, #
    Bubo_bubo == 1 & Otus_sunia == 1 ~ 4, #
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Tyto", "Since Athene", "Since Otus", "Since Bubo"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2, 4:6)]) +
  scale_y_continuous(limits = c(0, 610), expand = c(0,0), name = "Insertions") +
