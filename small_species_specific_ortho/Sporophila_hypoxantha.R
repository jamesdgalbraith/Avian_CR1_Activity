...
# set query species
species_name <- species_list$species_name[105]
...
for(i in c(1, 44, 60, 85, 103, 110, 113, 116, 117)){
...
as_tibble(data.frame(table(collated_info$Zonotrichia_albicollis,
                           collated_info$Taeniopygia_guttata, collated_info$Serinus_canaria,
                           collated_info$Passer_domesticus, collated_info$Geospiza_fortis))) %>%
  arrange(-Freq) %>%
  slice(1:20)
...
table(collated_info$Passer_domesticus)
# determine whether ancestral or recent (needs to be adjusted for each species)
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Zonotrichia_albicollis == 0 & Taeniopygia_guttata == 0 & Corvus_brachyrhynchos == 0 & Acanthisitta_chloris == 0 ~ 0,
    Zonotrichia_albicollis == 0 & Taeniopygia_guttata == 0 & Corvus_brachyrhynchos == 0 & Acanthisitta_chloris == 1 ~ 1,
    Zonotrichia_albicollis == 0 & Taeniopygia_guttata == 0 & Corvus_brachyrhynchos == 1 ~ 2,
    Zonotrichia_albicollis == 0 & Taeniopygia_guttata == 1 & Corvus_brachyrhynchos == 1 ~ 3,
    Zonotrichia_albicollis == 1 & Taeniopygia_guttata == 1 ~ 4, 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
species_name <- species_list$species_name[105]
genome_name <- species_list$genome_name[105]
