...
# set query species
species_name <- species_list$species_name[39]
...
collated_info_2 <- collated_info %>%
  # filter(Charadrius_alexandrinus %in% c(0, 1) & Recurvirostra_avosetta %in% c(0, 1) & Himantopus_leucocephalus %in% c(0, 1)) %>%
  mutate(state = case_when(
    Charadrius_alexandrinus == 0 & Recurvirostra_avosetta == 0 & Himantopus_leucocephalus == 0 ~ 0, # 
    Charadrius_alexandrinus == 0 & Recurvirostra_avosetta == 0 & Himantopus_leucocephalus == 1 ~ 1, #
    Charadrius_alexandrinus == 0 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 0 ~ 2, #
    Charadrius_alexandrinus == 0 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 1 ~ 3, # 
    Charadrius_alexandrinus == 1 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 1 ~ 4, # 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Himantopus", "Since Recurvirostra", "Since Himantopus and Recurvirostra", "Since Charadrius alexandrinus"),
...
ggplot(data = collated_info_3, aes(jc_dist, subfamily, fill = as.factor(subfamily))) + geom_density() + theme_bw()
...
species_name <- species_list$species_name[39]
genome_name <- species_list$genome_name[39]
...
collated_info_3_ranges <- collated_info_3 %>%
  ...
  mutate(start = ifelse(ostrand == "+", oend - 99, ostart - 20),
         end   = ifelse(ostrand == "+", oend + 20, ostart + 99)) %>%
  select(qseqid, seqnames, start, end, strand, subfamily, sseqid) %>%
  as_granges()

collated_info_3_tbl <- collated_info_3_tbl %>%
  inner_join(collated_info_3 %>% dplyr::select(subfamily, sseqid) %>% base::unique() %>% dplyr::rename(Var1 = sseqid))
