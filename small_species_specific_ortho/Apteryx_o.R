...
# set query species
species_name <- species_list$species_name[18]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Casuarius_casuarius == 0 & Apteryx_rowi == 0 & Apteryx_haastii == 0) ~ 1,
    (Casuarius_casuarius == 1 & Apteryx_rowi == 0 & Apteryx_haastii == 0) ~ 2,
    (Casuarius_casuarius == 1 & Apteryx_rowi == 1 & Apteryx_haastii == 0) ~ 3,
    (Apteryx_rowi == 1 & Apteryx_haastii == 1) ~ 4,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Casuarius", "Since A. rowi", "Since A. haasti"),
...
species_name <- species_list$species_name[18]
genome_name <- species_list$genome_name[18]
