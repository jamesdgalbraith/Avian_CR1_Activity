...
# set query species
species_name <- species_list$species_name[83]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Bubo_bubo == 0 & Athene_cunicularia == 0 & Tyto_alba == 0 ~ 0, #
    Bubo_bubo == 0 & Athene_cunicularia == 0 & Tyto_alba == 1 ~ 1, #
    Bubo_bubo == 0 & Athene_cunicularia == 1 & Tyto_alba == 1 ~ 2, #
    Bubo_bubo == 1 & Athene_cunicularia == 1 ~ 3, #
    TRUE ~ -1 # unclear
  )
  ) %>%
...
species_name <- species_list$species_name[83]
genome_name <- species_list$genome_name[83]
