...
# set query species
species_name <- species_list$species_name[1]
...
for(i in c(1, 44, 54, 69, 71, 85, 110)){
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Melopsittacus_undulatus == 0 & Falco_peregrinus == 0 & (Manacus_vitellinus == 0 | Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) ~ 1,
    Melopsittacus_undulatus == 0 & Falco_peregrinus == 1 & (Manacus_vitellinus == 0 | Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) ~ 2,
    Melopsittacus_undulatus == 1 & Falco_peregrinus == 1 & (Manacus_vitellinus == 0 | Corvus_brachyrhynchos == 0 | Taeniopygia_guttata == 0) ~ 3,
    Melopsittacus_undulatus == 1 & Falco_peregrinus == 1 & (Manacus_vitellinus == 1 & Corvus_brachyrhynchos == 1 & Taeniopygia_guttata == 1) ~ 4,
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Parrots", "Since Oscines/Suboscines"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1,2 ,4)]) +
  scale_y_continuous(limits = c(0, 450), expand = c(0,0), name = "Insertions") +
...
# species_name <- species_list$species_name[1]
# genome_name <- species_list$genome_name[1]
# 
