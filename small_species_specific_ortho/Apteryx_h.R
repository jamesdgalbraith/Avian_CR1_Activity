...
#set query species
species_name <- species_list$species_name[17]
...
reciprocal_blast_out <- read_tsv(paste0("extended_fastas/recip_blast/", species_name, ".out"),
  ...
  dplyr::select(qseqid, jc_dist, subfamily, sseqid, class_colour) %>%
  dplyr::rename(subfamily = subfamily)

# determine whether ancestral or recent (needs to be adjusted for each species)
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Casuarius_casuarius == 0 & Apteryx_rowi == 0 & Apteryx_owenii == 0) ~ 1,
    (Casuarius_casuarius == 0 & Apteryx_rowi == 0 & Apteryx_owenii == 0) ~ 2,
    (Casuarius_casuarius == 1 & Apteryx_rowi == 0 & Apteryx_owenii == 0) ~ 3,
    (Casuarius_casuarius == 1 & Apteryx_rowi == 1 & Apteryx_owenii == 0) ~ 4,
    (Apteryx_rowi == 1 & Apteryx_owenii == 1) ~ 5,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Casuarius", "Since A. rowi", "Since A. owenii"),
                    values = brewer.pal(n = 4, name = 'BrBG')) +
  scale_y_continuous(limits = c(0, 260), expand = c(0,0), name = "Insertions") +
...
collated_info_3_tbl <- tibble(data.frame(table(collated_info_3$sseqid))) %>%
  filter(Freq/sum(Freq) > 0.01) %>% mutate(sseqid = as.character(Var1))
collated_info_3 %>% select(subfamily, sseqid) %>% base::unique()
collated_info_3 <- collated_info_3 %>%
  filter(sseqid %in% collated_info_3_tbl$sseqid)

ggplot(data = collated_info_3, aes(jc_dist, fill = as.factor(sseqid))) + geom_histogram(binwidth = 0.005) +
  scale_x_continuous(name = "Jukes-Cantor distance", expand = c(0,0), limits = c(-0.005, 0.405)) + theme_bw()

to_align <- tibble(data.frame(table(collated_info_3$state, collated_info_3$cluster_name)))

# set species and genome names
species_name <- species_list$species_name[17]
genome_name <- species_list$genome_name[17]

genome_seq <- readDNAStringSet(paste0("~/Genomes/Birds/", species_name, "/", genome_name))
names(genome_seq) <- sub(" .*", "", names(genome_seq))
gc()

for(i in 1:nrow(to_align)){
  
  to_align_ranges <- collated_info_3 %>%
    filter(state == as.character(to_align$Var1[i]), cluster_name == as.character(to_align$Var2[i])) %>%
    dplyr::select(qseqnames, ostart, oend, ostrand) %>%
    dplyr::rename(seqnames = qseqnames, start = ostart, end = oend, strand = ostrand) %>%
    dplyr::mutate(start = ifelse(strand == "+", end - 99, start),
                  end = ifelse(strand == "+", end, start + 99))
  if(nrow(to_align_ranges) == 0 ){next()}
  if(nrow(to_align_ranges) > 500){to_align_ranges <- sample_n(to_align_ranges, 500)}
  
  to_align_ranges <- to_align_ranges%>%
    as_granges()
  ...
  if(length(to_align_ranges) < 3 ){next()}
  system(paste0("mafft --localpair --thread 12 temp/temp.fa > extended_fastas/new_class/alignments/", species_name, "#", as.character(to_align$Var2[i]),
                "#", as.character(to_align$Var1[i]),".fasta"))
  system(paste0("Gblocks extended_fastas/new_class/alignments/", species_name, "#", as.character(to_align$Var2[i]),
                "#", as.character(to_align$Var1[i]),".fasta -t d -b1", ceiling(length(to_align_seq)/4), " -b2 ", ceiling(length(to_align_seq)/4), "-b4 2 -b5 h"))
  
}


  
ggplot(data = reciprocal_blast_out, aes(jc_dist, fill = as.factor(subfamily))) + geom_histogram(binwidth = 0.005)
table(reciprocal_blast_out$subfamily)
