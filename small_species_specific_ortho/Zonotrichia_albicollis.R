...
# set query species
species_name <- species_list$species_name[116]
...
for(i in c(1, 4, 105, 91, 109, 110, 116, 117)){
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Sporophila_hypoxantha == 0 & Taeniopygia_guttata == 0 & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0) ~ 1, 
    Sporophila_hypoxantha == 0 & Taeniopygia_guttata == 0 & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1) ~ 2, 
    Sporophila_hypoxantha == 0 & Taeniopygia_guttata == 0 & Sturnus_vulgaris == 1 & (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0) ~ 3, 
    Sporophila_hypoxantha == 0 & Taeniopygia_guttata == 0 & Sturnus_vulgaris == 1 & (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1) ~ 4,
    Sporophila_hypoxantha == 0 & Taeniopygia_guttata == 1 & Sturnus_vulgaris == 1 ~ 5,
    Sporophila_hypoxantha == 1 & Taeniopygia_guttata == 1 & Sturnus_vulgaris == 1 ~ 6,
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(
    labels = c("Since Acanthisitta", "Since Corvus", "Since Zosterops", "Since Zonotrichia"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1, 4:6)]) +
...
species_name <- species_list$species_name[116]
genome_name <- species_list$genome_name[116]
...
collated_info_3_ranges <- collated_info_3 %>%
  select(qseqid, qseqnames, ostart, oend, ostrand, subfamily, sseqid, state) %>%
