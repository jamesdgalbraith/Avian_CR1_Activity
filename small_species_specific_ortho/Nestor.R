...
# set query species
species_name <- species_list$species_name[78]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Melopsittacus_undulatus == 0 & Strigops_habroptilis == 0) ~ 1,
    (Melopsittacus_undulatus == 1 & Strigops_habroptilis == 0) ~ 2,
    (Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1) ~ 3,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Melopsittacus", "Since Strigops"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1:2,4)]) +
  scale_y_continuous(limits = c(0, 2100), expand = c(0,0), name = "Insertions") +
...
species_name <- species_list$species_name[106]
genome_name <- species_list$genome_name[106]
