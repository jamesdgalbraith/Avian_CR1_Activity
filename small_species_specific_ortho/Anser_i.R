...
# set query species
species_name <- species_list$species_name[13]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Anas_zonorhyncha == 0 & Anser_cygnoides_domesticus == 0 & Anser_brachyrhynchus == 0 & Gallus_gallus == 0) ~ 0, # very ancestral
    (Anas_zonorhyncha == 0 & Anser_cygnoides_domesticus == 0 & Anser_brachyrhynchus == 0 & Gallus_gallus == 1) ~ 1, # ancestral
    (Anas_zonorhyncha == 1 & Anser_cygnoides_domesticus == 0 & Anser_brachyrhynchus == 0 & Gallus_gallus == 1) ~ 2, # since Anas
    (Anas_zonorhyncha == 1 & Anser_cygnoides_domesticus == 0 & Anser_brachyrhynchus == 1 & Gallus_gallus == 1) ~ 3, # indicus x cygnoides hybrid
    (Anas_zonorhyncha == 1 & Anser_cygnoides_domesticus == 1 & Anser_brachyrhynchus == 0 & Gallus_gallus == 1) ~ 4, # indicus x brachyrhynchus hybrid
    (Anas_zonorhyncha == 1 & Anser_cygnoides_domesticus == 1 & Anser_brachyrhynchus == 1) ~ 5, # since cygnoides+brachyrhynchus
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Anas", "Anser i. + Anser c.", "Anser i. + Anser b.", "Since Anser i./b."),
...
species_name <- species_list$species_name[13]
genome_name <- species_list$genome_name[13]
