...
species_list <-readr::read_tsv("~/Genomes/Birds/bird_genomes.tsv", col_names = c("species_name", "genome_name"))
work_dir <- "~/temp"
...
# set query species
species_name <- species_list$species_name[110]
...
for(i in c(1, 4, 44, 54, 69, 71, 91, 105, 109, 110, 116, 117)){
...
collated_info %>%
  filter(Zosterops_lateralis_melanops != -1, Zonotrichia_albicollis != -1, Sturnus_vulgaris != -1, Sporophila_hypoxantha != -1, Phylloscopus_trochilus_acredula != -1,  Melopsittacus_undulatus != -1,  Manacus_vitellinus != -1,  Falco_peregrinus != -1, Corvus_brachyrhynchos != -1, Acridotheres_javanicus != -1, Acanthisitta_chloris != -1, Taeniopygia_guttata != -1) %>%
  dplyr::select(-qseqnames, -ostart, -oend, -ostrand, -olength)
colnames(collated_info)
# determine whether ancestral or recent (needs to be adjusted for each species)
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0)  & Corvus_brachyrhynchos == 0 &
    #   Manacus_vitellinus == 0 & Acanthisitta_chloris == 0 & Melopsittacus_undulatus == 1 & Falco_peregrinus == 1 ~ 0,
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0)  & Corvus_brachyrhynchos == 0 &
    #   Manacus_vitellinus == 0 & Acanthisitta_chloris == 1 & Melopsittacus_undulatus == 1 ~ 1,
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0)  & Corvus_brachyrhynchos == 0 &
    #   Manacus_vitellinus == 1& Acanthisitta_chloris == 1 ~ 2,
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0)  & Corvus_brachyrhynchos == 1  & Manacus_vitellinus == 1 ~ 3, # since corvids
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0)  & Corvus_brachyrhynchos == 1  & Manacus_vitellinus == 0 ~ 3, # (deletion from crows)
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 &
    #   (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1)  & Corvus_brachyrhynchos == 1 ~ 4,
    # (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 1 &
    #   (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1) ~ 5,
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0) & Corvus_brachyrhynchos == 0 & Manacus_vitellinus == 0 ~ 0, # present in all
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0) & Corvus_brachyrhynchos == 0 & Manacus_vitellinus == 1 ~ 1, # present in all
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 0 | Zosterops_lateralis_melanops == 0) & Corvus_brachyrhynchos == 1 & Manacus_vitellinus == 1 ~ 2, # since corvids
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 0 & (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1) & Corvus_brachyrhynchos == 1 ~ 3, # since Pt/Zl
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 1 & (Phylloscopus_trochilus_acredula == 1 | Zosterops_lateralis_melanops == 1) ~ 4, # since sturnus
    (Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) & Sturnus_vulgaris == 1 ~ 5, # insertion since Za/Sp
    (Zonotrichia_albicollis == 1 | Sporophila_hypoxantha == 1) & Sturnus_vulgaris == 1 ~ 6, # insertion since Za/Sp
    TRUE ~ -1 # unclear
  )
  ) %>%
  inner_join(reciprocal_blast_out) %>%
  # filter(state != -1, state != 0) %>%
  base::unique()

table(collated_info_2$state, collated_info_2$subfamily)

table(collated_info_2[collated_info_2$state == 5,]$Sporophila_hypoxantha, collated_info_2[collated_info_2$state == 5,]$Zonotrichia_albicollis)

colnames(collated_info_2)
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(
    labels = c("Since Acanthisitta", "Since Manacus", "Since Corvus", "Since Zosterops", "Since Lonchura"),
    values = brewer.pal(n = 4, name = 'BrBG')) +
  scale_y_continuous(limits = c(0, 45), expand = c(0,0), name = "Insertions") +
...
collated_info_3_tbl <- as_tibble(as.data.frame(table(collated_info_3$subfamily))) %>%
  ...
  filter(Freq/sum(Freq) > 0.005, Freq > 1)

collated_info_3 <- collated_info_3 %>%
  filter(subfamily %in% collated_info_3_tbl$Var1)

ggplot(data = collated_info_3, aes(jc_dist, subfamily, fill = as.factor(subfamily))) + geom_violin(scale = "count") + theme_bw() +
  ...
  scale_x_continuous(expand = c(0,0), name = "Jukes-Cantor distance") +
...
# species_name <- species_list$species_name[110]
# genome_name <- species_list$genome_name[110]
# 
# genome_seq <- readDNAStringSet(paste0("~/Genomes/Birds/", species_name, "/", genome_name))
# names(genome_seq) <- sub(" .*", "", names(genome_seq))
# gc()
# 
# collated_info_3_ranges <- collated_info_3 %>%
#   group_by(subfamily) %>%
#   arrange(jc_dist) %>%
#   # dplyr::slice(1:200) %>%
#   ungroup() %>%
#   mutate(seqnames = qseqnames, strand = ostrand) %>%
#   mutate(start = ostart,
#          end   = oend) %>%
#   select(qseqid, seqnames, start, end, strand, subfamily, sseqid) %>%
#   as_granges()
# 
# collated_info_3_tbl <-tibble(as.data.frame(table(collated_info_3$subfamily))) %>% mutate(subfamily = as.character(Var1)) %>% filter(Freq > 1)
# 
# for(i in 1:nrow(collated_info_3_tbl)){
#   
#   to_align_ranges <- collated_info_3_ranges %>%
#     filter(subfamily == collated_info_3_tbl$subfamily[i])
#   
...
#   
#   system(paste0("mafft --localpair --thread 12 temp/temp.fa > extended_fastas/new_class/alignments/", species_name, "#", collated_info_3_tbl$subfamily[i], ".fasta"))
#   
# }