...
# set query species
species_name <- species_list$species_name[7]
...
collated_info_2 <- collated_info %>%
  # filter(Amazona_aestiva != -1, Amazona_vittata != -1, Strigops_habroptilis != -1, Melopsittacus_undulatus != -1) %>%
  mutate(state = case_when(
    Amazona_aestiva == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 0 & Strigops_habroptilis == 0 ~ 0, # 
    Amazona_aestiva == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 0 & Strigops_habroptilis == 1 ~ 1, # 
    Amazona_aestiva == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1 ~ 2, # 
    Amazona_aestiva == 1 & Amazona_vittata == 0 & Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1 ~ 3, # 
    Amazona_aestiva == 1 & Amazona_vittata == 1 & Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1 ~ 4, # 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Strigops", "Since Melopsittacus", "Since A. aestiva", "Since A. vittata"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2, 4:6)]) +
  scale_y_continuous(limits = c(0, 1600), expand = c(0,0), name = "Insertions") +
...
species_name <- species_list$species_name[7]
genome_name <- species_list$genome_name[7]
