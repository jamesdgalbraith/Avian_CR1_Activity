...
# set query species
species_name <- species_list$species_name[22]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Bubo_bubo == 0 & Tyto_alba == 0 ~ 0, #
    Bubo_bubo == 0 & Tyto_alba == 1 ~ 1, #
    Bubo_bubo == 1 & Tyto_alba == 1 ~ 2, #
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Tyto", "Since Bubo"),
...
species_name <- species_list$species_name[22]
genome_name <- species_list$genome_name[22]
