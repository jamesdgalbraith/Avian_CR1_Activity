...
# set query species
species_name <- species_list$species_name[24]
...
for(i in c(24, 25, 22, 83, 107, 114)){
...
as_tibble(data.frame(table(collated_info$Bubo_blakistoni, collated_info$Bubo_bubo, collated_info$Athene_cunicularia, collated_info$Otus_sunia, collated_info$Strix_occidentalis, collated_info$Tyto_alba))) %>%
  filter(Freq > 1) %>%
  mutate(Var1 = as.integer(as.character(Var1)), Var2 = as.integer(as.character(Var2)), Var3 = as.integer(as.character(Var3)),
         Var4 = as.integer(as.character(Var4)), Var5 = as.integer(as.character(Var5)), Var6 = as.integer(as.character(Var6))) %>%
  mutate(total = Var1 + Var2 + Var3 + Var4 + Var5 + Var6) %>%
  arrange(-Freq) %>%
  slice(1:20)
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Bubo_bubo == 0 & Strix_occidentalis == 0 & Otus_sunia == 0 & Athene_cunicularia == 0 & Tyto_alba == 0 ~ 0, #
    Bubo_bubo == 0 & Strix_occidentalis == 0 & Otus_sunia == 0 & Athene_cunicularia == 0 & Tyto_alba == 1 ~ 1, #
    Bubo_bubo == 0 & Strix_occidentalis == 0 & Otus_sunia == 0 & Athene_cunicularia == 1 & Tyto_alba == 1 ~ 2, #
    Bubo_bubo == 0 & Strix_occidentalis == 0 & Otus_sunia == 1 & Athene_cunicularia == 1 ~ 3, #
    Bubo_bubo == 0 & Strix_occidentalis == 1 & Otus_sunia == 1 ~ 4, #
    Bubo_bubo == 1 & Strix_occidentalis == 1 ~ 5, #
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Tyto", "Since Athene", "Since Otus", "Since Strix", "Since B. bubo"),
                    values = brewer.pal(n = 6, name = 'BrBG')) +
  scale_y_continuous(limits = c(0, 600), expand = c(0,0), name = "Insertions") +
...
species_name <- species_list$species_name[24]
genome_name <- species_list$genome_name[24]
