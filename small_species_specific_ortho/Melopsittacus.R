...
# set query species
species_name <- species_list$species_name[71]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Psittacula_krameri == 0 & Strigops_habroptilis == 0) ~ 1,
    (Psittacula_krameri == 0 & Strigops_habroptilis == 1) ~ 2,
    (Psittacula_krameri == 1 & Strigops_habroptilis == 1) ~ 3,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Strigops", "Since Psittacula"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1,3:4)]) +
...
species_name <- species_list$species_name[71]
genome_name <- species_list$genome_name[71]
