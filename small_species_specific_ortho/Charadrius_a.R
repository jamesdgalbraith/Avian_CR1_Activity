...
# set query species
species_name <- species_list$species_name[38]
...
collated_info_2 <- collated_info %>%
  # filter(Charadrius_vociferus %in% c(0, 1) & Recurvirostra_avosetta %in% c(0, 1) & Himantopus_leucocephalus %in% c(0, 1)) %>%
  mutate(state = case_when(
    Charadrius_vociferus == 0 & Recurvirostra_avosetta == 0 & Himantopus_leucocephalus == 0 ~ 0, # 
    Charadrius_vociferus == 0 & Recurvirostra_avosetta == 0 & Himantopus_leucocephalus == 1 ~ 1, #
    Charadrius_vociferus == 0 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 0 ~ 2, #
    Charadrius_vociferus == 0 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 1 ~ 3, # 
    Charadrius_vociferus == 1 & Recurvirostra_avosetta == 1 & Himantopus_leucocephalus == 1 ~ 4, # 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Himantopus", "Since Recurvirostra", "Since Himantopus and Recurvirostra", "Since Charadrius vociferus"),
...
species_name <- species_list$species_name[38]
genome_name <- species_list$genome_name[38]
