...
# set query species
species_name <- species_list$species_name[49]
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Casuarius_casuarius == 0 & (Apteryx_haastii == 0 | Apteryx_owenii == 0 | Apteryx_rowi == 0)) ~ 1,
    (Casuarius_casuarius == 0 & (Apteryx_haastii == 1 | Apteryx_owenii == 1 | Apteryx_rowi == 1)) ~ 2,
    (Casuarius_casuarius == 1 & (Apteryx_haastii == 1 | Apteryx_owenii == 1 | Apteryx_rowi == 1)) ~ 3,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Apteryx", "Since Dromaius"),
...
species_name <- species_list$species_name[35]
genome_name <- species_list$genome_name[35]
