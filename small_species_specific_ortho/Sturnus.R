...
# set query species
species_name <- species_list$species_name[109]
...
for(i in c(1, 4, 44, 105, 109, 110, 116, 117)){
...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    Zosterops_lateralis_melanops == 0 & Corvus_brachyrhynchos == 0 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) ~ 1, 
    Zosterops_lateralis_melanops == 0 & Corvus_brachyrhynchos == 1 & (Taeniopygia_guttata == 0 | Zonotrichia_albicollis == 0 | Sporophila_hypoxantha == 0) ~ 2, 
    Zosterops_lateralis_melanops == 0 & Corvus_brachyrhynchos == 1 & (Taeniopygia_guttata == 1 | Zonotrichia_albicollis == 1 | Sporophila_hypoxantha == 1) ~ 3, 
    Zosterops_lateralis_melanops == 1 & Corvus_brachyrhynchos == 1 & (Taeniopygia_guttata == 1 | Zonotrichia_albicollis == 1 | Sporophila_hypoxantha == 1) ~ 4, 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Since Acanthisitta", "Since Manacus", "Since Corvus", "Since Taeniopygia"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1,2,3, 4)]) +
  scale_y_continuous(limits = c(0, 40), expand = c(0,0), name = "Insertions") +
...
# species_name <- species_list$species_name[109]
# genome_name <- species_list$genome_name[109]
# 
