...
# set query species
species_name <- species_list$species_name[29]
...
collated_info_2 <- collated_info %>%
  # filter(Calidris_pygmaea %in% c(0, 1) & Scolopax_mira %in% c(0, 1) & Limosa_lapponica_baueri %in% c(0, 1)) %>%
  mutate(state = case_when(
    Calidris_pygmaea == 0 & Scolopax_mira == 0 & Limosa_lapponica_baueri == 0 ~ 0, # 
    Calidris_pygmaea == 0 & Scolopax_mira == 0 & Limosa_lapponica_baueri == 1 ~ 1, #
    Calidris_pygmaea == 0 & Scolopax_mira == 1 & Limosa_lapponica_baueri == 0 ~ 2, # 
    Calidris_pygmaea == 0 & Scolopax_mira == 1 & Limosa_lapponica_baueri == 1 ~ 3, # 
    Calidris_pygmaea == 1 & Scolopax_mira == 1 & Limosa_lapponica_baueri == 1 ~ 4, # 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
collated_info_4 <- collated_info %>% inner_join(reciprocal_blast_out)

table(collated_info_4$subfamily)
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Limosa",  "Since Scolopax", "Since Scolopax and Limosa", "Since Calidris pygmea"),
...
species_name <- species_list$species_name[29]
genome_name <- species_list$genome_name[29]
