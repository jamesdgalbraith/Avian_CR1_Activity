...
# set query species
species_name <- species_list$species_name[6]
...
as_tibble(data.frame(table(collated_info$Amazona_aestiva, collated_info$Amazona_collaria, collated_info$Psittacula_krameri,
                           collated_info$Melopsittacus_undulatus, collated_info$Strigops_habroptilis))) %>%
  filter(Freq > 1) %>%
  mutate(Var1 = as.integer(as.character(Var1)), Var2 = as.integer(as.character(Var2)), Var3 = as.integer(as.character(Var3)),
         Var4 = as.integer(as.character(Var4)), Var5 = as.integer(as.character(Var5))) %>%
  filter(Var3 != -1, Var4 != -1, Var5 == 1) %>%
...
collated_info_2 <- collated_info %>%
  # filter(Amazona_collaria != -1, Amazona_vittata != -1, Strigops_habroptilis != -1, Melopsittacus_undulatus != -1) %>%
  mutate(state = case_when(
    Amazona_collaria == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 0 & Strigops_habroptilis == 0 ~ 0, # 
    Amazona_collaria == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 0 & Strigops_habroptilis == 1 ~ 1, # 
    Amazona_collaria == 0 & Amazona_vittata == 0 & Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1 ~ 2, #
    Amazona_collaria == 1 & Amazona_vittata == 1 & Melopsittacus_undulatus == 1 & Strigops_habroptilis == 1 ~ 3, # 
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Strigops", "Since Melopsittacus", "Since other Amazona"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2, 5:6)]) +
  scale_y_continuous(limits = c(0, 1500), expand = c(0,0), name = "Insertions") +
...
collated_info_3 <- collated_info_2 %>%
  filter(state %in% c(2, 3, 4))

collated_info_3_tbl <- as_tibble(as.data.frame(table(collated_info_3$sseqid))) %>%
  mutate(Var1 = as.character(Var1)) %>%
  filter(Freq/sum(Freq) > 0.005, Freq > 1)

collated_info_3 <- collated_info_3 %>%
  filter(sseqid %in% collated_info_3_tbl$Var1)
...
ggplot(data = collated_info_3, aes(jc_dist, fill = as.factor(sseqid))) + geom_histogram(binwidth = 0.005)

# set species and genome names
species_name <- species_list$species_name[6]
genome_name <- species_list$genome_name[6]
...
collated_info_3_ranges <- collated_info_3 %>%
  ...
  dplyr::slice(1:200) %>%
  ungroup() %>%
  mutate(subfamily = paste0(sseqid, "#", subfamily), seqnames = qseqnames, strand = ostrand) %>%
  mutate(start = ifelse(ostrand == "+", oend - 79, ostart - 20),
         end   = ifelse(ostrand == "+", oend + 20, ostart + 79)) %>%
  select(qseqid, seqnames, start, end, strand, subfamily, sseqid) %>%
  as_granges()

collated_info_3_tbl <- collated_info_3_tbl %>%
  inner_join(collated_info_3 %>% dplyr::select(subfamily, sseqid) %>% base::unique() %>% dplyr::rename(Var1 = sseqid)) %>% filter(Freq > 1)
