...
# set query species
species_name <- species_list$species_name[114]
...
collated_info_2 <- collated_info %>%
  # filter(Bubo_bubo > -1, Athene_cunicularia > -1, Otus_sunia > -1, Strix_occidentalis > -1, Tyto_alba > -1) %>%
  mutate(state = case_when(
    Picoides_pubescens == 0 & Bubo_bubo == 0 ~ 0, #
    Picoides_pubescens == 1 & Bubo_bubo == 0 ~ 0, #
    Picoides_pubescens == 1 & Bubo_bubo == 1 ~ 3, #
    TRUE ~ -1 # unclear
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral",  "Since Tyto", "Since Athene", "Since Bubo"),
                    values = brewer.pal(n = 6, name = 'BrBG')[c(1:2, 5:6)]) +
  scale_y_continuous(limits = c(0, 680), expand = c(0,0), name = "Insertions") +
...
species_name <- species_list$species_name[114]
genome_name <- species_list$genome_name[114]
...
for(i in 1:nrow(collated_info_3_tbl)){
  ...
  to_align_ranges <- collated_info_3_ranges %>%
    filter(subfamily == collated_info_3_tbl$Var1[i])
  ...
  system(paste0("mafft --localpair --thread 12 temp/temp.fa > extended_fastas/new_class/alignments/", species_name, "#", collated_info_3_tbl$Var1[i], ".fasta"))
  
}
readDNAMultipleAlignment("extended_fastas/new_class/alignments/Tyto_alba##CR1-Y.fasta")
