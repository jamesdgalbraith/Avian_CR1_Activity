...
# set query species
species_name <- species_list$species_name[102]
...
collated_info_2 <- collated_info %>%
  ...
  mutate(state = case_when(
    (Calidris_pugnax == 0 | Calidris_pygmaea == 0) & Limosa_lapponica_baueri == 0 ~ 0, # ancestral
    (Calidris_pugnax == 0 | Calidris_pygmaea == 0) & Limosa_lapponica_baueri == 1 ~ 1, # since divergence
    (Calidris_pugnax == 1 | Calidris_pygmaea == 1) & Limosa_lapponica_baueri == 1 ~ 2, # since divergence
    TRUE ~ -1 # unclear
  )
  ) %>%
