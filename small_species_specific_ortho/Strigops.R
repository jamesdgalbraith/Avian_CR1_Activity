...
collated_info_2 <- collated_info %>%
  mutate(state = case_when(
    (Melopsittacus_undulatus == 0 & Nestor_notabilis == 0) ~ 1,
    (Melopsittacus_undulatus == 1 & Nestor_notabilis == 0) ~ 2,
    (Melopsittacus_undulatus == 1 & Nestor_notabilis == 1) ~ 3,
    TRUE ~ -1
  )
  ) %>%
...
divergence_plot <- ggplot(data = collated_info_2, aes(jc_dist, fill = as.factor(state))) + geom_histogram(binwidth = 0.005) +
  ...
  scale_fill_manual(labels = c("Ancestral", "Since Melopsittacus", "Since Nestor"),
                    values = brewer.pal(n = 4, name = 'BrBG')[c(1:2,4)]) +
  scale_y_continuous(limits = c(0, 2400), expand = c(0,0), name = "Insertions") +
